...
const excelPath = path.resolve(__dirname, '../../Group B  EDUCA.xlsx');
...
try {
  ...
  for (const [sheetName, fileName] of Object.entries(SHEET_MAPPING)) {
    if (workbook.Sheets[sheetName]) {
      ...
      const questions = extractQuestions(workbook.Sheets[sheetName], fileName.replace('groupB-', '').replace('.json', ''));
